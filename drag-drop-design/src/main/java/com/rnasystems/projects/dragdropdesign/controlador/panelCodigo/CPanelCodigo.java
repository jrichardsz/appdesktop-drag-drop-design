 ...
 * @author RM-RCMJ
 */
public class CPanelCodigo implements ActionListener {

    public CPanelCodigo(PanelCodigo panelCodigo) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == jButtonActualizar) {
            ...
            //iteramos la cantidad de panelgrids
            
            HashMap<String, DragDropComponent> listaPanelGrid = ((CPanelDisenio) (AppUtil.sessionMap.get("CPanelDisenio"))).getListaPanelGrid();
            
            Iterator it = listaPanelGrid.entrySet().iterator();
            IcePanelGrid temp = null;

            while (it.hasNext()) {
                
                Map.Entry ee = (Map.Entry) it.next();
                temp = (IcePanelGrid) ee.getValue();
                
                GeneraCodigoPanelGrid codigoPanelGrid = new GeneraCodigoPanelGrid();
                codigoPanelGrid.inicializaVariables(temp);
                lineas_codigo = codigoPanelGrid.getLineasCodigo();
    ...
    public void insertaComponentesModoDiv() {
        String inicioDiv = "\t\t<div id=\"contatiner\">";
    ...
    public String insertaCommandButton(int c,DragDropComponent componente){
        String temp = "\t\t\t<div id=\"comp_"+c+"\">\n\t\t\t\t<ice:commandButton  "
                + "value=\""+componente.getText()+"\" "
                //+ "style=\"width:"+componente.getWeight()+"px;height:"+componente.getHeight()+"px;\""
                + " ></ice:commandButton>\n\t\t\t</div>";
        return temp;
    }
